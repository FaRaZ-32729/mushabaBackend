...
const messageSchema = new mongoose.Schema({
    ...
    type: {
        type: String,
        enum: ['text', 'voice', 'image', 'video', 'location', 'customLocation', 'personalLocation', 'system'],
        required: true
    },
    content: {
        // Text messages
    ...
    // FileSystem-first media fields (optional - no breaking changes)
    mediaId: {
        type: String,
        required: false // Optional field for FileSystem media ID
    },
    isLocal: {
        type: Boolean,
        default: false // Whether media is stored locally in FileSystem
    },
    syncStatus: {
        type: String,
        enum: ['pending', 'synced', 'failed'],
        default: 'synced' // Default to synced for existing messages
    }
}, {
    timestamps: true
});
