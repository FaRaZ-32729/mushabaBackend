...
const removeUser = async (req, res) => {
    try {
        ...
        if (connection.users.length === 1 && connection.users[0].role === 'owner') {
            // ✅ COMPREHENSIVE CLEANUP: Delete all connection-related data FIRST
            try {
                ...
                // 4. Delete Private Chats between connection users
                const connectionUserIds = connection.users.map(u => u.userId.toString());

                // Find all private chats between any two users in this connection
                const privateChats = await Chat.find({
                    type: 'private',
                    'participants.userId': { $all: connectionUserIds },
                ...
                if (privateChats.length > 0) {
                    ...
                    console.log(`[CONNECTION_CLEANUP] Deleted ${deletedPrivateMessages.deletedCount} Messages from private chats`);

                    // 4.2. Then delete the private chats themselves
                    const deletedPrivateChats = await Chat.deleteMany({
                        _id: { $in: privateChatIds }
                    });
                    console.log(`[CONNECTION_CLEANUP] Deleted ${deletedPrivateChats.deletedCount} Private chats`);
                } else {
                    console.log(`[CONNECTION_CLEANUP] No private chats found between connection users`);
                }

                // 5. Delete ActivityLogs
                const deletedActivityLogs = await ActivityLog.deleteMany({ connectionId: connectionId });
                console.log(`[CONNECTION_CLEANUP] Deleted ${deletedActivityLogs.deletedCount} ActivityLogs`);

                // 6. Delete ConnectionRequests
                const deletedConnectionRequests = await ConnectionRequest.deleteMany({ connectionId: connectionId });
                console.log(`[CONNECTION_CLEANUP] Deleted ${deletedConnectionRequests.deletedCount} ConnectionRequests`);

                // 7. Delete Notifications (connection-related only)
                const deletedNotifications = await Notification.deleteMany({ 'data.connectionId': connectionId });
                ...
                const updatedUsers = await User.updateMany(
                    {
                        $or: [
                            ...
                            { 'markedLocations.connectionId': connectionId },
                    ...
                    },
                    {
                        $unset: {
                            ...
                            'markedLocations.connectionId': 1,
                            'currentConnection.connectionId': 1
                        }
                    }
                );
                console.log(`[CONNECTION_CLEANUP] Updated ${updatedUsers.modifiedCount} User documents`);

                // 9. Delete the group chat AFTER deleting its messages
                if (groupChat) {
                    await groupChat.deleteOne();
                    console.log('Group chat deleted with connection:', groupChat._id);
                }

                console.log(`[CONNECTION_CLEANUP] Comprehensive cleanup completed successfully for connection: ${connectionId}`);
            } catch (cleanupError) {
                console.error(`[CONNECTION_CLEANUP] Error during cleanup for connection ${connectionId}:`, cleanupError);
                // Continue execution - don't fail the connection deletion if cleanup fails
            }
            ...
            if (req.app.get('io')) {
                ...
                // Also emit connectionUpdated for MainScreen handling
                const removedUser = await User.findById(targetUserId);
                io.to(`user:${targetUserId}`).emit('connectionUpdated', {
...
const getSingleConnection = async (req, res) => {
    try {
        const { connectionId } = req.params;
    ...
    try {
        ...
        if (req.app.get('io')) {
            ...
            for (const id of userIds) {
                ...
                io.to(`user:${id}`).emit('connectionUpdated', {
                    type: 'connection_deleted',
                    connectionId: connection._id,
                    connectionDeleted: true,
                    userName: user.name
        ...
        if (action === 'approve') {
            try {
                ...
                if (webSocketService && webSocketService.emitNotificationToUser) {
                    ...
                    console.log('[DEBUG] WebSocket service IO instance:', !!webSocketService.io);
                    console.log('[DEBUG] WebSocket service IO engine clients:', webSocketService.io?.engine?.clientsCount);
    ...
    try {
        ...
        if (connection.users.length === 1 && connection.users[0].role === 'owner') {
            ...
            if (req.app.get('io')) {
                ...
                io.to(`user:${userId}`).emit('connectionUpdated', {
                    type: 'connection_deleted',
                    connectionId: connection._id,
                    connectionDeleted: true,
                ...
                // Also emit to the remaining user (owner) that connection was deleted
                const remainingUser = connection.users.find(u => u.userId.toString() !== userId);
                if (remainingUser) {
                    io.to(`user:${remainingUser.userId}`).emit('connectionUpdated', {
                        type: 'connection_deleted',
                        connectionId: connection._id,
                        connectionDeleted: true,
                        userName: leavingUser.name
...
const getUserWithTime = async (req, res) => {
    try {
        const { connectionId } = req.params;
        const userId = req.user.id;
...
const getCurrnetUserConnections = async (req, res) => {
    try {
...
const transferOwnership = async (req, res) => {
    try {
        const { connectionId } = req.params;
        const { newOwnerId, password, verificationCode } = req.body;
        const currentUserId = req.user.id;
        ...
        if (currentUserDoc.isGoogleUser) {
            ...
            const isValidCode = emailVerificationService.verifyCode(
                ...
                verificationCode,
                currentUserDoc.verificationCodeExpires
            );
            ...
            if (currentUserDoc.verificationCodePurpose !== 'ownership_transfer') {
                return res.status(401).json({
                    success: false,
            ...
            console.log('Google user verification successful for ownership transfer:', currentUserId);
        } else {
