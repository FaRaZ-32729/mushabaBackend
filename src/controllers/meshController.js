const connections = require('../models/connectionSchema');
const User = require('../models/userSchema');
const { ConnectionLocation } = require('../models/locationSchema');
const { getUserLocations } = require('./locationController');

// In-memory store for mesh network data (separate from existing locationController)
const meshUserLocations = {};

// 🚀 NEW: Database update function for mesh locations (similar to locationController) //use in api
...
const relayOfflineUserData = async (req, res) => {
    try {
        ...
        for (const user of otherUsers) {
            ...
            if (!userLocation) {
                const regularLocations = getUserLocations();
...
const handleOfflineUserResponse = async (req, res) => {
    try {
...
const getMeshLocations = async (req, res) => {
    try {
        return res.json({
            success: true,
...
// Expose meshUserLocations for potential future use // not in use
const getMeshUserLocations = () => meshUserLocations;

module.exports = {
    relayOfflineUserData,
    handleOfflineUserResponse,
    getMeshLocations
}