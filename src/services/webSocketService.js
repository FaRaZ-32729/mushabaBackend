...
class WebSocketService {
    ...
    emitNotificationToUser(userId, notification) {
        try {
            console.warn(`🔔 [WEBSOCKET] Attempting to emit notification to user: ${userId}`, {
                ioExists: !!this.io,
                roomId: `user:${userId}`,
                notificationType: notification.type,
                notificationData: notification
            });
            console.warn(`🔔 [WEBSOCKET] Emitting to room: user:${userId}`);

            this.io.to(`user:${userId}`).emit('newNotification', {
                notification,
                timestamp: new Date().toISOString()
            });
            console.warn(`🔔 [WEBSOCKET] Notification emitted to user: ${userId}`);
        } catch (error) {
